...
class DatabaseMessageStore implements MessageStore {
    ...
    private static class MQTTDatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase database) {
            String createArrivedTableStatement = "CREATE TABLE " + ARRIVED_MESSAGE_TABLE_NAME + "(" + MqttServiceConstants.MESSAGE_ID
                    + " TEXT PRIMARY KEY, " + MqttServiceConstants.CLIENT_HANDLE + " TEXT, " + MqttServiceConstants.DESTINATION_NAME + " TEXT, "
                    + MqttServiceConstants.PAYLOAD + " BLOB, " + MqttServiceConstants.QOS + " INTEGER, " + MqttServiceConstants.RETAINED + " TEXT, "
    ...
    public String storeArrived(String clientHandle, String topic, MqttMessage message) {
        ...
        traceHandler.traceDebug(TAG,
                "storeArrived: inserted message with id of {" + id + "} - Number of messages in database for this clientHandle = " + count);
        return id;
    }
    ...
    public boolean discardArrived(String clientHandle, String id) {
        ...
        try {
            rows = db.delete(ARRIVED_MESSAGE_TABLE_NAME, MqttServiceConstants.MESSAGE_ID + "=? AND " + MqttServiceConstants.CLIENT_HANDLE + "=?",
                    selectionArgs);
        } catch (SQLException e) {
        ...
        return new Iterator<StoredMessage>() {
...

            {
                ...
                if (clientHandle == null) {
                    ...
                    c = db.query(ARRIVED_MESSAGE_TABLE_NAME, null, MqttServiceConstants.CLIENT_HANDLE + "=?", selectionArgs, null, null,
                            "mtimestamp ASC");
                }
