...
//        Notification notification = new Notification.Builder(this).setContentTitle(String.format("%s运行中", proName))
//                .setSmallIcon(R.mipmap.ic_class_logo, 0)
//                .build();
...
//        startForeground(1, notification);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            String proName = "MQTT测试程序";
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                String id = "push_screen";
                String name = "forground_push";
                NotificationManager manager = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
                NotificationChannel channel = new NotificationChannel(id, name, NotificationManager.IMPORTANCE_HIGH);
                manager.createNotificationChannel(channel);
                notification = new Notification.Builder(this, id)
                        .setContentTitle(String.format("%s正在运行", proName))
                        .setSmallIcon(R.mipmap.ic_logo_noti)
                        .setPriority(Notification.PRIORITY_MAX)
                        .build();
            } else {
                notification = new Notification.Builder(this)
                        .setContentTitle(String.format("%s正在运行", proName))
                        .setSmallIcon(R.mipmap.ic_logo_noti, 0)
                        .setPriority(Notification.PRIORITY_MAX)
                        .build();
                notification.flags = Notification.FLAG_FOREGROUND_SERVICE;
            }
    ...
    private class NetworkConnectionIntentReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            String name = "IdealClass";
            //这句话不添加应用标识会导致警告Tag name should use a unique prefix followed by a colon
            //意思就是 你的tag 名字要使用 app名:tag 的形式 比如：IdealClass
            WakeLock wl = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, String.format("%sMQTT", name));
            wl.acquire();
